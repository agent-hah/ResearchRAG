...
def mock_langchain_components(mocker):
    ...
    mock_pinecone_store = mocker.patch('rag.services.rag_service.PineconeVectorStore')
    mock_pinecone_client = mocker.patch('rag.services.rag_service.Pinecone')
    ...
    mock_pinecone_store.return_value = mock_vs_instance
    
    # For search, return dummy docs
    mock_doc = MagicMock()
    mock_doc.page_content = "dummy text"
    mock_doc.metadata = {"literature_id": 1, "filename": "dummy.pdf"}
    mock_vs_instance.similarity_search_with_relevance_scores.return_value = [(mock_doc, 0.9)]
    
    # For get_stats
    mock_pc_instance = MagicMock()
    mock_index = MagicMock()
    mock_stats = MagicMock()
    mock_stats.namespaces = {"test_user": MagicMock(vector_count=10)}
    mock_index.describe_index_stats.return_value = mock_stats
    mock_pc_instance.Index.return_value = mock_index
    mock_pinecone_client.return_value = mock_pc_instance
...
def test_index_literature(mock_langchain_components):
    ...
    with patch('rag.services.rag_service.time.sleep'):
        service = RAGService("test_user")
        literature = Literature.objects.create(
            filename="dummy.pdf",
            file_path="dummy.pdf",
            file_size=1024,
            processing_status=ProcessingStatus.PENDING, user_id='test_user'
        )
        ...
        mock_langchain_components['vs_instance'].add_texts.assert_called_once()
...
def test_get_stats(mock_langchain_components):
    ...
    Literature.objects.create(
        ...
        file_path="dummy.pdf",
        file_size=1024,
        processing_status=ProcessingStatus.INDEXED, user_id='test_user'
    )
...
def test_delete_literature_index(mock_langchain_components):
    ...
    mock_langchain_components['vs_instance'].delete.assert_called_once_with(filter={"literature_id": literature.id})
...
def test_search_literature_with_filter_and_max_chunks(mock_langchain_components):
    ...
    mock_langchain_components['vs_instance'].similarity_search_with_relevance_scores.return_value = [
    ...
    mock_langchain_components['vs_instance'].similarity_search_with_relevance_scores.assert_called_once()
    kwargs = mock_langchain_components['vs_instance'].similarity_search_with_relevance_scores.call_args[1]
    assert kwargs['filter'] == {"literature_id": {"$in": [1]}}

@pytest.mark.django_db
def test_search_literature_exception(mock_langchain_components):
    service = RAGService("test_user")
    mock_langchain_components['vs_instance'].similarity_search_with_relevance_scores.side_effect = Exception("Test error")
...
def test_get_stats_exception(mock_langchain_components):
    service = RAGService("test_user")
    mock_langchain_components['index'].describe_index_stats.side_effect = Exception("Test error")
...
def test_delete_literature_chunks_exception(mock_langchain_components):
    service = RAGService("test_user")
    mock_langchain_components['vs_instance'].delete.side_effect = Exception("Test error")
...
def test_reindex_all(mock_langchain_components):
    with patch('rag.services.rag_service.time.sleep'), patch('rag.services.rag_service.PDFProcessor.extract_text', return_value="dummy text"):
        service = RAGService("test_user")
        Literature.objects.create(filename="dummy1.pdf", file_path="dummy1.pdf", file_size=1024, processing_status=ProcessingStatus.INDEXED, user_id='test_user')
        Literature.objects.create(filename="dummy2.pdf", file_path="dummy2.pdf", file_size=1024, processing_status=ProcessingStatus.INDEXED, user_id='test_user')
...
def test_get_rag_service(mock_langchain_components):
    service1 = get_rag_service("test_user")
    service2 = get_rag_service("test_user")
